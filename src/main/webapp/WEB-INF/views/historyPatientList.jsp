...
<%@ taglib uri="http://www.springframework.org/tags/form" prefix="form"%>
	...
	<jsp:include page="header.jsp" /> <!-- Include the header.jsp page on build -->
		...
		<div id="content" style="text-align:center">
			...
			<h2>Patient List:</h2>
			<p><b>To view a Patients' Medical history Click on Patients' Name.</b></p>
				...
				<tr>
					...
					<c:forEach var="pat" items="${patient}">
						<tr>
							<td><a href = '<c:url value="/historyController/${pat.id}/"/>'>${pat.name}</a></td><td>${pat.address1}</td><td>${pat.address2}</td><td>${pat.address3}</td><td>${pat.address4}</td><td>${pat.dateOfBirth}</td><td>${pat.phoneNumber}</td>
						</tr>
					</c:forEach>   
				</table> 
			</div>>